...
(F) - full access"
 ...
 icacls_permissions_ntds_folder = json( command: "icacls '#{ntds_database_logs_files_path}' | ConvertTo-Json").params.map { |e| e.strip }[0..-3].map{ |e| e.gsub("#{ntds_database_logs_files_path} ", '') }
 #Command Gets the Location of the Property Required
 ...
 icacls_permissions_ntds_dsa_file = json( command: "icacls '#{ntds_dsa_file_path}' | ConvertTo-Json").params.map { |e| e.strip }[0..-3].map{ |e| e.gsub("#{ntds_dsa_file_path} ", '') }
